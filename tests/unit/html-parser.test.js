...
describe('HTML Parser Module', () => {
  ...
  it('should parse HTML and extract paragraphs', () => {
    const html = fs.readFileSync(
      path.join(fixturePath, 'lectures', 'soul_god_and_religion.html'),
      'utf-8'
    );
    const result = parseHTML(html);
    ...
    const hasRelevantContent = result.paragraphs.some(
      p =>
  ...
  it('should extract title and language from HTML', () => {
    const html = fs.readFileSync(
      path.join(fixturePath, 'lectures', 'the_hindu_religion.html'),
      'utf-8'
    );
    const result = parseHTML(html);
